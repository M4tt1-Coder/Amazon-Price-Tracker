...
- Install important packages in your virtual environment ( more below! )
...
#### General uploading of changes

First step will be to stage all current made changes. Open a new terminal in the **`Amazon-Price-Tracker`** - folder:

```bash
  git add .
```

Then, we need to create a new commit for the staged changes:

```bash
  git commit -m "<Describing title>" -m "<Explanation for changes>"
```

As the last step push your commit to the remote repository:
